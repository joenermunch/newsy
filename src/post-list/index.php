 ...
 */
function newsy_dynamic_post_list_init() {
	// Register the block and specify the render function.
	register_block_type( NEWSY_BLOCKS_DIR_PATH . '/build/post-list', array(
		'render_callback' => 'render_dynamic_post_list',
	) );
}
add_action( 'init', 'newsy_dynamic_post_list_init' );
 ...
 */
function render_dynamic_post_list( $attributes ) {
	...
	$content = '<div class="post-list-container">';
	...
	if ( $post_query->have_posts() ) {
		...
		while ( $post_query->have_posts() ) {
			...
			$content .= '<li class="post-item-secondary">';
